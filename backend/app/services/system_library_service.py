...
def get_system_document_download(document_id: str, user: dict | None = None) -> dict:
    ...
    if storage_path:
        try:
            ...
            # Supabase storage may raise malformed StorageApiError/KeyError on 404.
            # Do not emit a traceback for a recoverable missing object; fall back to
            # the open-access URL if the metadata has one, otherwise return 404.
            logger.warning("System document storage object missing/unavailable for document_id=%s path=%s: %s", document_id, storage_path, exc)
