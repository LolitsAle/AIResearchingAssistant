...
from app.db.models import ChatMessage, Document, DocumentChunk, Note, Workspace
...
def upload_source(workspace_id: str, file: UploadFile = File(...), db: Session = Depends(get_db)):
    if not db.query(Workspace).filter(Workspace.id == workspace_id).first(): raise AppError('Workspace không tồn tại', 404)
