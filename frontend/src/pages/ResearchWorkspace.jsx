...
function DocumentsPanel({ documents, selectedDocumentIds, canUseContext, onToggleDocument, onSelectAllReady, onCreateSession, onUpload, uploadProgress, uploadError, onDismissUploadError, loadingDocuments, leftTab, onLeftTab, onPrompt, onGenerateFlashcards, onGenerateQuiz, quickLoading, onOpenLibrary, onDeleteDocument, flashcardCount, onFlashcardCount, quizCount, onQuizCount }) {
  ...
  return <>
    ...
    {leftTab === "documents" ? <>
      ...
      {!canUseContext ? <div className="rw-warning">Hãy tạo/mở phiên và chọn ít nhất một tài liệu ready trước khi dùng prompt hoặc tính năng nhanh.</div> : <>
...
function ChatPanel({ messages, input, onInput, onSubmit, loading, loadingLabel, historyLoading, selectedDocuments, session, hasDocuments, canChat, emptyState, onPrepareUpload, onPickSession, onClear, onExport, onRegenerate, onCopy, onSaveNote, savedMessageIds, savingNoteId, onShowSources, diagnostics, mode, onModeChange, onPrompt, showAllPrompts, setShowAllPrompts }) {
  ...
  const showPrompts = canChat;
  const inputDisabled = loading || historyLoading || !canChat;
  const handleKey = (e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); if (canChat) onSubmit(); } };
  return <><div className="rw-chat-head"><div><div className="rw-session-name">Đang ở phiên: {session?.title || "Chưa chọn phiên"}</div><div className="rw-doc-pill">Tài liệu tham chiếu: {selectedDocuments.length ? selectedDocuments.map((d) => d.filename).join(", ") : "Chưa chọn"}</div></div><div className="rw-chat-actions"><div className="rw-mode-segment" aria-label="Chế độ truy xuất"><button className={mode === "strict" ? "active" : ""} type="button" title="Chỉ trả lời khi nguồn đủ liên quan" onClick={() => onModeChange("strict")}>Chặt chẽ</button><button className={mode === "exploratory" ? "active" : ""} type="button" title="Cho phép trả lời rộng hơn kèm cảnh báo" onClick={() => onModeChange("exploratory")}>Khám phá</button></div><button className="rw-soft-btn" type="button" onClick={onExport} disabled={!session?.id}>Export</button><button className="rw-soft-btn" type="button" onClick={onClear} disabled={!session?.id || loading || historyLoading}>Xóa chat</button></div></div><div className={`rw-messages rw-scroll ${!historyLoading && messages.length === 0 && !showPrompts ? "is-empty-state" : ""}`}>
    {!historyLoading && messages.length === 0 && !showPrompts && <div className="rw-empty centered"><h2>{emptyState?.title || "Chưa có phiên nghiên cứu"}</h2><p>{emptyState?.description || "Hãy tải tài liệu lên hoặc chọn một phiên nghiên cứu để bắt đầu."}</p>{!session?.id && !hasDocuments && <div className="rw-empty-ctas"><button className="rw-primary" type="button" onClick={onPrepareUpload}>Tải tài liệu</button><button className="rw-soft-btn" type="button" onClick={onPickSession}>Chọn phiên cũ</button></div>}{session?.id && !selectedDocuments.length && <div className="rw-warning" style={{ marginTop: 12 }}>Phiên này chưa có tài liệu được chọn.</div>}</div>}
    {!historyLoading && messages.length === 0 && showPrompts && <div className="rw-empty"><h2>Research Workspace</h2><p>Chọn tài liệu, mở/tạo phiên, rồi đặt câu hỏi. Prompt nhanh chỉ điền vào input khi bấm.</p><div className="rw-prompts">{visibleGroups.map((group) => <div className="rw-prompt-group" key={group.group}><div className="rw-prompt-title">{group.group}</div><div className="rw-prompt-buttons">{group.prompts.slice(0, showAllPrompts ? 3 : 1).map((prompt) => <button className="rw-chip" type="button" key={prompt} onClick={() => onPrompt(prompt)}>{prompt}</button>)}</div></div>)}<button className="rw-soft-btn" type="button" onClick={() => setShowAllPrompts(!showAllPrompts)}>{showAllPrompts ? "Thu gọn" : "Xem thêm"}</button></div></div>}
    ...
    {loading && <div className="rw-stage">● {loadingLabel || "Đang truy xuất nguồn..."}</div>}<div ref={bottomRef} /></div><div className="rw-input-area"><div className="rw-textarea-wrap"><textarea className="rw-textarea rw-scroll" value={input} onChange={(e) => onInput(e.target.value)} onKeyDown={handleKey} placeholder={canChat ? "Đặt câu hỏi về tài liệu..." : "Hãy chọn phiên hoặc tải tài liệu trước khi hỏi AI."} disabled={inputDisabled} maxLength={1000} /><button className="rw-primary" type="button" onClick={onSubmit} disabled={!input.trim() || inputDisabled}>{historyLoading ? "Đang tải" : loading ? "Đang gửi" : "Gửi"}</button></div><p className="rw-hint">Enter gửi · Shift+Enter xuống dòng · đổi tab/panel không gọi AI.</p></div></>;
}
...
export default function ResearchWorkspace() {
  ...
  const hasReadyContext = Boolean(activeSession?.id && selectedDocuments.some((doc) => doc.processing_status === "ready" || doc.is_vector_ready));
  const chatEmptyState = !activeSession?.id && documents.length === 0
    ? { title: "Chưa có phiên nghiên cứu", description: "Hãy tải tài liệu lên hoặc chọn một phiên nghiên cứu để bắt đầu." }
    : !activeSession?.id
      ? { title: "Chưa có phiên nghiên cứu", description: "Chọn tài liệu và tạo phiên nghiên cứu để bắt đầu chat." }
      : selectedDocuments.length === 0
        ? { title: "Phiên này chưa có tài liệu được chọn", description: "Hãy chọn ít nhất một tài liệu đã sẵn sàng để bắt đầu chat." }
        : { title: "Tài liệu chưa sẵn sàng", description: "Vui lòng chờ tài liệu được index xong trước khi hỏi AI." };
  ...
  const handleUpload = async (files) => {
    ...
    setUploadProgress(1); try { const result = await api.uploadDocuments(notebookId, valid, token, setUploadProgress); const uploaded = (result?.uploaded || []).map(normalizeDocument); const queued = (result?.queued || []).map(normalizeDocument); const warnings = result?.storage_warnings || result?.warnings || []; queued.forEach((doc) => subscribeIndexingJob(doc.indexing_job_id || doc.indexing_job?.id)); if (warnings.length) { const warningMessage = warnings[0]?.message || "Tài liệu đã được xử lý tạm thời nhưng chưa lưu bền do thiếu storage bucket. Vui lòng kiểm tra cấu hình Supabase bucket."; setUploadError(warningMessage); showToast("error", warningMessage); } else showToast("success", queued.length ? "Upload xong, tài liệu đang được index nền." : "Upload hoàn tất."); await loadDocuments(); if (activeSession?.id && uploaded.length) { const next = [...new Set([...selectedDocumentIds, ...uploaded.filter((d) => d.processing_status === "ready").map((d) => d.id)])]; await updateActiveSessionDocuments(next); } } catch (err) { setUploadError(err.message || "Upload thất bại."); } finally { setTimeout(() => setUploadProgress(0), 800); }
  };
