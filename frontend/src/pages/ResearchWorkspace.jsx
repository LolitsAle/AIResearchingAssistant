...
function DocumentsPanel({ documents, selectedDocumentIds, onToggleDocument, onSelectAllReady, onCreateSession, onUpload, uploadProgress, uploadError, onDismissUploadError, loadingDocuments, leftTab, onLeftTab, onPrompt, onGenerateFlashcards, onGenerateQuiz, quickLoading, onOpenLibrary, onDeleteDocument, flashcardCount, onFlashcardCount, quizCount, onQuizCount }) {
  ...
  return <>
    ...
    {leftTab === "documents" ? <>
      ...
      <div className="rw-card"><div className="rw-card-title">Flashcard</div><p className="rw-muted">Chọn số lượng 1–5, sau đó tạo bộ flashcard có thể lật Front/Back.</p><label className="rw-range-row">Số flashcard: <strong>{flashcardCount}</strong><input type="range" min="1" max="5" value={flashcardCount} onChange={(e) => onFlashcardCount(Number(e.target.value))} /></label><button className="rw-primary" type="button" disabled={quickLoading} onClick={onGenerateFlashcards}>{quickLoading ? "Đang tạo..." : "Tạo flashcard"}</button></div>
      <div className="rw-card"><div className="rw-card-title">Quiz</div><p className="rw-muted">Chọn số lượng 1–5, làm quiz A/B/C/D và xem giải thích ngay dưới đáp án.</p><label className="rw-range-row">Số câu hỏi: <strong>{quizCount}</strong><input type="range" min="1" max="5" value={quizCount} onChange={(e) => onQuizCount(Number(e.target.value))} /></label><button className="rw-primary" type="button" disabled={quickLoading} onClick={onGenerateQuiz}>{quickLoading ? "Đang tạo..." : "Tạo quiz"}</button></div>
    </div>}
  </>;
}
...
function NotesPanel({ notes, loading, filter, onFilter, onSaveEdit, onDelete, onExportMarkdown, onCitation }) {
  const [detail, setDetail] = useState(null);
  ...
  const openDetail = (note) => { setDetail(note); setEditing(false); setDraft({ title: note.title || "", content: note.content || "" }); };
  ...
  return <div className="rw-right-content rw-scroll">
    ...
    {filtered.map((note) => <button type="button" className="rw-card" key={note.id} onClick={() => openDetail(note)}>
      <div className="rw-row" style={{ justifyContent: "space-between" }}><div className="rw-card-title">{note.title || "Ghi chú"}</div><span className="rw-muted">Mở chi tiết</span></div>
...
function FlashcardModal({ data, onClose }) {
  ...
  return <Modal title={data.title || "Flashcards"} onClose={onClose}>
    ...
    <div className="rw-flashcard-stage">
      ...
      <p className="rw-muted">{index + 1}/{cards.length} · tối đa 5 flashcard</p>
    </div>
  </Modal>;
}
