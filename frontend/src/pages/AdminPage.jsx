...
const STYLES = `
  ...
  .admin-drop { border: 1px dashed rgba(212,182,111,.5); border-radius: 18px; padding: 24px; text-align: center; background: rgba(0,0,0,.18); color: #bfb4a3; }
  ...
  .admin-status { margin-top: 12px; display: flex; align-items: center; gap: 8px; color: #d7c494; font-size: 13px; }
...
export default function AdminPage() {
  ...
  const handleImport = async (event) => {
    ...
    if (!file) {
      setMessage('Vui lòng chọn file PDF, DOCX, TXT hoặc MD.');
      return;
    }
    setProgress(0);
    setStatus('uploading');
    ...
    try {
      const result = await api.importSystemDocument({ ...form, file }, token, setProgress);
      setStatus('indexing');
      ...
      setStatus('ready');
      setMessage('Import hoàn tất: tài liệu đã parse, tạo metadata và vector ready.');
    } catch (err) {
      setStatus('failed');
  ...
  return (
    <div className="admin-page">
      ...
      <section className="admin-hero">
        ...
        <p>Import tài liệu để người dùng có thể tìm kiếm, tải xuống và sử dụng trong Không gian Nghiên cứu.</p>
      </section>

      <div className="admin-grid">
        <form className="admin-panel" onSubmit={handleImport}>
          <h2>Import tài liệu</h2>
          <label className="admin-drop" onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); setFile(event.dataTransfer.files?.[0] || null); }}>
            <UploadCloud size={32} />
            <strong>Kéo thả file hoặc chọn file</strong>
            <span>{file ? file.name : 'Hỗ trợ PDF, DOCX, TXT, MD'}</span>
            <input type="file" accept=".pdf,.docx,.txt,.md" hidden onChange={(event) => setFile(event.target.files?.[0] || null)} />
          </label>
          ...
          <button type="submit" className="admin-button" disabled={busy}>{busy ? <Loader2 className="spin" size={16} /> : <UploadCloud size={16} />} Import vào Thư viện Hệ thống</button>
              ...
              <article className="admin-doc" key={doc.id}>
                <div>
                  ...
                  <div className="admin-meta">
                    <span>{doc.filename}</span><span>•</span><span>{doc.file_type}</span><span>•</span><span>{doc.category || 'Khác'}</span><span>•</span><span>{doc.is_vector_ready ? 'vector ready' : 'processing'}</span><span>•</span><span>{doc.created_at ? new Date(doc.created_at).toLocaleString('vi-VN') : '—'}</span>
                  </div>
