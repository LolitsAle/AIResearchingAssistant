...
const AcademicNotepad = forwardRef(function AcademicNotepad({ value, onChange, onSave, saveStatus = 'idle', storage = 'database' }, ref) {
  ...
  return (
    <aside ref={ref} className="al-notepad">
      <div className="al-notepad-head">
        <div><strong>Markdown Notepad</strong><span>{saveStatus === 'saving' ? 'Đang lưu…' : saveStatus === 'saved' ? 'Đã lưu' : saveStatus === 'error' ? 'Lỗi lưu' : 'Autosave sau 1–2 giây idle'}{storage === 'memory_fallback' ? ' · Memory fallback (chưa bền DB)' : ''}</span></div>
